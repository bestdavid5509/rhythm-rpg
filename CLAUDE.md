...
| `CurrentMp`, `MaxMp`, `IsDefending`, `BeckoningTargets`, `IsAbsorber` | player-only; null/default on enemies. `BeckoningTargets` is a `HashSet<Combatant>` of enemies this player has beckoned — multiple concurrent Beckons stack across turns; each enemy's entry is consumed individually in `SelectEnemyAttack` when its turn fires. Beckon's picker excludes enemies already in this set. `IsAbsorber` gates absorbed-move learning and Beckon submenu visibility. |
...
Magic and cast attacks spawn their timing circle at `_magicCircleAnchor` — a static formation-anchored position computed once at scene init in `BuildInitialParties`. **X is hardcoded at viewport center** (`MagicCircleX = 960f`, post-C11.3) for predictable input placement regardless of formation X variance — though the formation centroids are already symmetric within 2.5px of viewport center, so the hardcode coincides with the natural center. **Y is derived** from the midpoint of player formation center Y (`(PlayerFrontAnchor.Y + PlayerBackAnchor.Y) / 2`) and enemy formation center Y (`_enemyParty[0].AnimSpriteOrigin.Y + EnemyBackRowYOffset/2`), lifted by `MagicCircleYOffset` (`-50f` — interactively tuned to sit in the upper portion of the formation Y range, "amidst the action" rather than floating above it; the slight overlap with taller sprites is the cost of keeping magic input close to the action). The Y derivation auto-preserves the relative-above-formation property under formation lifts (e.g. C11.3's −50px FloorY lift). Stable for the entire battle; the Phase 1 → Phase 2 transition's ~14px slot-0 anchor shift is below the noise floor for circle position.
...
### Formation Anchor Lockstep (Phase 6 C11.3)

`FloorY`, `PlayerFrontAnchor`, `PlayerBackAnchor`, `PlayerFrontAnchorRect`, `PlayerBackAnchorRect` are five constants on `BattleTest` that must shift by the same Y magnitude when the formation is moved vertically. The four anchor Y values are hand-tuned to match `FloorY`-derived positions at scene init (slot 0 sprite center: `FloorY - playerFrameH * 3 * 0.5`; the Rect anchors retain the legacy `-40` Y delta from their AnimSprite counterparts). When `FloorY` changes — as in C11.3's `-50` lift (750 → 700) — all four anchors must shift in lockstep or the player formation diverges from the floor line and the ColorRects diverge from the sprites. Enemy positions derive from `FloorY` at runtime in `BuildEnemySpriteFrames` so they auto-shift with no constant update. Future refactor opportunity: derive all four player anchors from `FloorY` at scene init so the lockstep self-maintains; flagged but out of Phase 6 scope.

### Damage Number Positioning (Phase 6 C11.3)

`ComputeDamageOrigin(Combatant)` derives spawn position from the sprite (not the ColorRect). X = `unit.AnimSpriteOrigin.X` (sprite center). Y = `unit.AnimSpriteOrigin.Y - renderedHeight * 0.25 + SpriteContentYOffset - DamageNumberMargin` — step up to an upper-body anchor (0.25 × rendered height above sprite center, less aggressive than the original frame-top 0.5 multiplier), step down by `SpriteContentYOffset` for transparent-space allowance, then step up by the margin. Default `DamageNumberMargin = 25f`. The 0.25 multiplier was interactively tuned to bring warrior / 8 Sword Warrior numbers down out of "way above the head" placement — taller sprites have proportionally smaller transparent-space allowances than the Knight, so the 0.5 frame-top anchor floated their numbers too high. 0.25 lands warrior / boss numbers in upper-body / sword-art region (slight overlap with sword art accepted per design call); Knight numbers land in shoulder/upper-chest area. Sprite-derived placement gives consistent placement across sprite sizes (Knight 240px rendered, Warrior 390px, 8 Sword Warrior 480px). Uses `AnimSpriteOrigin` (rest-position snapshot) not `AnimSprite.GlobalPosition` (live tween position) so damage numbers stay anchored at the combatant's resting position even during slam / hop-in tweens — preserves the rest-position invariant. Five callsites consume the helper unchanged: basic offensive, magic per-circle, combo per-pass, enemy-on-player, parry-counter slash. The parry-counter damage number itself uses a separate in-body formula (see "Sprite Content Y Offset" below).

### Effect Spawn and Hop-in Midpoint Sprite-Derived Positioning (Phase 6 C11.3)

`BattleSystem.SpawnEffectSprite`'s `targetCenter` (the spawn baseline for attack visual effects) derives from `target.AnimSpriteOrigin + (0, SpriteContentYOffset)` — the rest-position sprite center plus the calibration-restore offset (see "Sprite Content Y Offset" below). Pre-C11.3 it used `target.Origin + target.PositionRect.Size / 2f` (ColorRect-center), which carried a ~40-50px Y offset from the sprite center (the long-standing "Cure target-circle positioning quirk" / damage-number quirk from Phase 3.3). The C11.3 formation lift would have doubled the gap to ~90-100px without the migration; effects would have spawned at sprite knee/foot height. Post-migration with the calibration restore, effects align with the visible body across all slots both sides.

`ComputeCameraMidpoint(attacker, defender)` derives Y from sprite-positions: attacker close-stance Y via the existing `ComputeAnimSpriteCloseY` helper (the same one `PlayHopIn` uses for the AnimSprite tween destination), defender Y from `defender.AnimSpriteOrigin`. Both Y values lifted by `SpriteContentYOffset` to match the calibration baseline. X stays ColorRect-derived (`_sequenceAttackerClosePos.X + attacker.PositionRect.Size.X / 2f` and `defender.Origin.X + defender.PositionRect.Size.X / 2f`) because the X dimension of the ColorRect-vs-sprite gap was negligible — the bug was purely Y. Used by hop-in attacks for camera zoom destination and timing-prompt circle position.

### Sprite Content Y Offset (Phase 6 C11.3)

`SpriteContentYOffset` (`internal const float`, currently `52f`) captures the empirical offset from sprite frame center down to the effective sprite-positioning baseline (~visible body center). The pre-C11.3 ColorRect-center sat ~40px below the sprite frame center on the player side, ~44px on the enemy side; `.tres`-authored `activeOffset` values for `SpawnEffectSprite` and the `ComputeCameraMidpoint` Y formula were calibrated against that ColorRect-center baseline. The C11.3 ColorRect→sprite migration switched to `AnimSpriteOrigin` (frame center) which removed this implicit offset; the constant restores it without re-introducing the ColorRect dependency. Interactively tuned to `52f` — slightly below the pre-C11.3 empirical 40-44 baseline, pulling hop-in circles and effect spawns down by ~12px relative to the bare calibration baseline to land closer to upper-body center. `internal` so `BattleSystem.cs` can reference it across the partial-class boundary (mirrors the `OverlayBottomInset` pattern).

Three consumers reference `SpriteContentYOffset`:
- `SpawnEffectSprite`'s `targetCenter` baseline (per the subsection above).
- `ComputeCameraMidpoint`'s Y formula for both attacker and defender.
- `ComputeDamageOrigin` uses it as a "frame-top → visible-content-top" allowance for transparent space above the visible head — different semantic, same magnitude empirically.

If verification surfaces meaningful asymmetry between the effect-spawn / midpoint use case (frame-center → body-center) and the damage-number use case (frame-top → content-top), split into separate `SpriteContentCenterOffset` and `SpriteContentTopOffset` constants in a follow-up tuning commit. Single-constant decision matches the "tunable interactive" philosophy of the C11 chunk.

Counter-attack damage uses a separate hand-tuned in-body formula (`BattleAnimator.cs PlayParryCounter`, ~`0.3 × renderedHeight + 50f`) — intentional divergence from `ComputeDamageOrigin`'s above-head anchor for the counter's "big impact" feel. Don't unify with `ComputeDamageOrigin`; the divergence is a design choice.
...
After a menu pick, `EnterSelectingTarget(CombatantSide, MenuContext)` enters the `SelectingTarget` state. Pool is alive combatants on the named side, sorted so each side's primary sort axis matches its dominant visual axis: enemies use Y-asc primary / X-asc tie-break (two-row grid — top row left-to-right, then bottom row), players use X-asc primary / Y-asc tie-break (single ↘ diagonal — leftmost-to-rightmost across the formation). Default starting target is pool index 0 — topmost-leftmost enemy or leftmost player. Singleton pools auto-confirm at entry — pointer never renders. For multi-target pools, `ui_left` / `ui_right` cycle through the pool with wraparound (cycling only mutates `_targetPool` / `_targetPoolIndex` / `_selectedTarget` / pointer position + per-sprite highlight, not attack-identity state on the launcher closure). `battle_confirm` invokes the captured `_pendingActionLauncher` against the resolved `_selectedTarget`; `battle_cancel` routes back to the originating `MenuContext`. Cure dispatches to `CombatantSide.Player` so allies (including self) are valid recipients; offensive attacks dispatch to `CombatantSide.Enemy`.

**Selection indicator (Phase 6 C11.1):** the sole active "selected target" cue is the yellow per-sprite highlight driven by `target_amount` on `CombatantOverlay.gdshader`. `ApplyTargetHighlight` / `ClearTargetHighlight` (mirroring C8's `ApplyActiveSpriteTint` pattern) fade the uniform via per-Combatant `TargetTween`. Lifecycle hooks: Apply on `EnterSelectingTarget` (default target) and on cycle (new target); Clear on cycle (previous target), `ConfirmTargetSelection` and `CancelTargetSelection`. Phase 2 transition's `SwapToPhase2` clears `target_amount` alongside the other shader uniforms. Composition order in the shader: `tint → active → target → flash` — yellow renders over the white-active wash so cure-on-self stacks cleanly; flash overrides everything (preserves the C8 invariant). Flash color is configurable via the `flash_color` uniform — currently purple (`Color(0.7, 0.3, 1.0)`) for the learnable signal written by `FlashCombatantLearnable`; defaults to white if a caller leaves it unset. The active wash itself "breathes" via a `TIME`-driven sine wave applied as an additive offset on `active_amount` before the mix — frequency and amplitude exposed as the inspector-tunable `active_breathing_hz` (0.5 Hz default) and `active_breathing_amplitude` (0.07 default) uniforms, smoothstep-gated on `active_amount` so non-active combatants don't flicker. The C# tween machinery (`ApplyActiveSpriteTint` / `ClearActiveSpriteTint`) is unchanged; breathing is purely a shader-side modulation of the held baseline. `ActiveTintAmount` was bumped from C8's 0.15 to 0.20 alongside the breathing pass — the eye accepts a higher peak when the wash dynamically falls back to a lower trough each cycle (effective swing 0.13 ↔ 0.27) rather than being statically pinned at maximum. Apply uses `TargetTintFadeSec` (0.08s); Clear uses the shorter `TargetTintClearSec` (0.04s) — asymmetric so rapid cycling shows one highlighted combatant at a time without ghost trails. The `TargetPointer` node and `SnapTo` formula stay wired but are gated off behind the `ShowTargetPointer` flag (`false` in C11.1) — the highlight does the job alone, and frame-top vs visible-content-top discrepancies make precise pointer placement unreliable without per-sprite content-top authoring. Re-enable by flipping the flag to `true`; the formula uses a 10%-of-rendered-height anchor above sprite center (`-(FrameHeight * AnimSpriteScale.Y * 0.1 + HeadMarginPixels)`) — empirically tuned to land the tip near the visible character body for the current sprite roster, since frame-top placement floats above the visible head due to transparent space at the top of sprite frames.

### Per-target Death Handling

`KillCombatant(Combatant)` runs the death sequence for any combatant whose HP reaches zero — sets `IsDead`, fades the strip card (both sides), plays the death animation, schedules `ScheduleBossRevealIfPhase1` for enemy deaths (internally gated on Phase 1 + non-multi-character via `IsPhaseTransitionPending`), and Removes the dying enemy from every player's `BeckoningTargets` set so no stale entry can fire a redirect. Idempotent on `IsDead`. Called from every `TakeDamage` site after damage applies — basic offensive ([BattleTest.cs](Scripts/Battle/BattleTest.cs)), offensive magic per-circle, combo per-pass, enemy-attack-on-player, parry counter ([BattleAnimator.cs](Scripts/Battle/BattleAnimator.cs)). Game-over branches (`OnFinalSlashFinished`, `BeginComboMissRetreat`, `OnEnemySequenceCompleted`, `OnPlayerMagicSequenceCompleted`, `OnEtherSequenceCompleted` `_pendingGameOver` paths) no longer fire kill work inline — KillCombatant has already run at the damage site. Those branches handle only the end-of-battle UX (retreat, idle, end-label, music fade).
...
**Lethal-counter death-animation deferral** (`_deferredEnemyDeath` field on `BattleTest` + `KillCombatant`'s `playAnimationNow` parameter): when the warrior is the hop-in attacker and a perfect-parry counter kills it, the raw flow (death animation kicks at slash-finished while `PlayTeardown` concurrently slides the AnimSprite back to origin) puts the dying body + explosion at mid-slide positions while the boss reveal spawns statically at origin — a 60-100px geometric gap that no spawn-timing or parallel-tween could fully cover. `PlayParryCounter` detects the case (lethal AND `_battleSystem.CurrentAttackIsHopIn` AND `IsPhaseTransitionPending()`) and calls `KillCombatant(playAnimationNow: false)`: state mutates at slash-finished (IsDead, strip fade, BeckoningTargets cleanup) but the visual sequence (sound + "death" animation + `ScheduleBossRevealIfPhase1`) defers. `ProceedAfterHopInAnim`'s lethal branch consumes `_deferredEnemyDeath` by passing a `PlayEnemyDeathSequence` callback to `PlayTeardown` so the death animation + reveal fire on `tween.Finished` at origin. The warrior holds its last hurt frame through the 350ms teardown slide-back, then plays the full death animation at origin with the reveal materializing co-located. **The deferral is gated to the phase-transition case only** — regular enemy lethal counters (e.g. 4v8 where `Phase2EnemyData` is null, or any non-warrior counter-kill) keep the concurrent death-during-slide behavior, which reads as a recoil-and-collapse when no boss reveal follows.
...
6. 0.5s pause → construct a `BattleDialogue` with a single `DialogueLine` (Speaker = "The Harbinger", Text = "How tedious.", AutoAdvanceSeconds = 2.0) and call `PlayDialogue` on it; `StartPhase2Music()` fires concurrently so the Phase 2 theme cue lands with the Harbinger's voice. `_phase2Dialogue.DialogueCompleted` hooks `OnPhase2DialogueCompleted` which calls `SwapToPhase2()` and `QueueFree`s the dialogue node. `SwapToPhase2()` finalises state: resets the enemy Combatant (`MaxHp`, `CurrentHp`, `IsDead = false`, reassigns `Data`/`Name`), updates the enemy name label, calls `UpdateHPBars()`, clears per-fight state on the player Combatant (`BeckoningTargets.Clear()`, `IsDefending = false`) and BattleTest (`_parryClean`, `_pendingGameOver`, hop-in rendezvous flags, `_lastAttackIndex`). `_absorbedMoves` is NOT reset — per-move-type absorb tracking persists across the phase transition so Phase 2's learnable is absorbable regardless of Phase 1 state.
...
|---|---|---|
| `StageExtentSize`   | `Vector2(2208, 1248)` | Originally sized as 6× the ansimuz GreenForest pixel-art native (368×208); current default texture is the painted Nidhoggn Battlebacks set scaled to fit |
...
The stage extends 144px beyond the 1920×1080 viewport horizontally and 84px vertically — giving the camera room to zoom and pan during hop-in close-ups without exposing off-stage area. Both backgrounds (the legacy dark-blue ColorRect and the opt-in painted-art `BackgroundLayer` Sprite2D) span the stage extent.

**`UseBackground` `[Export] bool`** toggles between the two backgrounds. Default `false` → dark-blue ColorRect visible, painted background hidden. `true` → painted background visible, dark-blue ColorRect hidden. Both always exist in the scene tree (`Background` ColorRect, single `BackgroundLayer` Sprite2D); `_Ready` writes mutual visibility based on the export value. Camera math is toggle-independent — it operates against the stage extent constants regardless of which background is showing. The off-default is deliberate during the post-Phase-6 polish window: current formation X anchors cluster the player/enemy formations at the viewport edges (tuned against the dark-blue background's lack of implied geography); the painted backgrounds expect the action to sit more centrally within the stage area — see `docs/phase-6-plan.md` §10 "Forest background formation re-tuning" for the pending re-tune that will flip the default to `true`.

**Painted-art background** (when `UseBackground = true`): single `Sprite2D` node parented to the `BattleTest` root with `Position = (960, 540)`, `Scale = (2.00966, 2.00966)` (uniform fit to vertical extent; horizontal overflows ~5px each side into the stage-extent reserve), `TextureFilter = LINEAR` (`texture_filter = 2` — smooths the ~2× upscale of painted art; contrasts with the NEAREST filter that the prior pixel-art layer architecture used), `ZIndex = -1` (below the Background ColorRect's z=0 and the entire formation/effects/pointer z-stack — formation 0..14, hop-in attacker 15, effects 3, TargetZone 32). Texture currently wired to `Dungeon.png` from the Nidhoggn OpenGameArt Battlebacks set (10 painted environments, CC0; the other 9 — Cave, DaytimeForest, DeepForest, Desert, DesertTown, EnemyFort, GoldenForest, RedForest, WinterForest — sit in the same folder for future per-encounter variety); swap the active texture via a one-line ext_resource change in `BattleTest.tscn`. Source attribution lives in `Assets/Backgrounds/OpenGameArtBattlebacks/source.txt` (per-folder convention) and a row in `CREDITS.md` Art table. The earlier ansimuz GreenForest three-layer composite is archived at `Assets/Backgrounds/GreenForestLayered/` with no active code references — retained in case the layered-parallax approach is revisited.
...
### Known debug-window quirk: cold-launch resolution

First run of the game after fully closing and reopening Godot may render at a slightly undersized resolution (~1228×691 or similar) with visible gray void at the window edges. Resizing the window once during the run, or running the game a second time within the same Godot session, fixes the rendering. This is a debug-window-only interaction between Godot's window override (1280×720), the editor's debug toolbar overhead in non-embedded mode, and `canvas_items` stretch initialization. Exported builds are not affected. Two fix attempts (explicit `window/stretch/aspect="keep"` in `project.godot`; `window_height_override` bumped 720 → 750 to accommodate the ~30px toolbar) were tried and reverted — neither produced a clean fix without workaround feel. Investigation deferred; revisit during the portable build work (item 3 on the project roadmap) to confirm exported builds are clean.
